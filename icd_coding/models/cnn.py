...
class CNN(BaseModel):
    ...
    def __init__(self, vocab, args=None, verbose=True):
        super(CNN, self).__init__(vocab, args, verbose)
    ...
    def build_model(self, output_shape):
        ...
        text_vector = self.vectorize_layer_voc(text_input)
        embedd_output = keras.layers.Embedding(len(self.vectorize_layer_voc.get_vocabulary()), embedding_dim, mask_zero=True)(text_vector)
        embedd_output = keras.layers.Dropout(dropout)(embedd_output)
        conv = keras.layers.Conv1D(filters, kernel_size, padding='valid', activation='relu')(embedd_output)
