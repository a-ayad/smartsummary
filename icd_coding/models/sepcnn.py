...
class SepCNN:
    ...
    def build_model(self, output_shape):
        ...
        x = keras.layers.SeparableConv1D(128, kernel_size, padding='same', activation='relu')(x)
        x = keras.layers.SeparableConv1D(128, kernel_size, padding='same', activation='relu')(x)
        x = keras.layers.MaxPooling1D()(x)
        x = keras.layers.Dropout(dropout)(x)
        x = keras.layers.SeparableConv1D(256, kernel_size, padding='same', activation='relu')(x)
        x = keras.layers.SeparableConv1D(256, kernel_size, padding='same', activation='relu')(x)
