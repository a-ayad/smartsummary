...
class EnsCNNLSTMAttention:
    ...
    def build_model(self, output_shape):
        ...
        att_weights_cnn, att_cnn = Attention(output_shape, name='attention')(cnn1)
        att_cnn = keras.layers.Dropout(dropout)(att_cnn)

        # Bi-LSTM
        lstm = keras.layers.Bidirectional(keras.layers.LSTM(lstm_dim, return_sequences=True))(x)
        # x = keras.layers.Bidirectional(keras.layers.LSTM(64, return_sequences=True))(x)
        att_weights_lstm, att_lstm = Attention(output_shape)(lstm)
        att_lstm = keras.layers.Dropout(dropout)(att_lstm)

        # concat = keras.layers.Concatenate(axis=-1)([att_cnn, att_lstm])

        label_li = []

        for i in range(output_shape):
            label_li.append(keras.layers.Dense(1, activation='sigmoid')(att_cnn[::, i]+att_lstm[::, i]))
