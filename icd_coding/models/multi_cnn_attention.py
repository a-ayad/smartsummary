...
class Multi_CAML:
    ...
    def build_model(self, output_shape):
        ...
        cnn1 = keras.layers.Conv1D(300, 3, padding='valid', activation='relu')(x)
        att1 = CNN_Attention(output_shape)(cnn1)
        att1 = keras.layers.Dropout(dropout)(att1)

        cnn2 = keras.layers.Conv1D(300, 5, padding='valid', activation='relu')(x)
        att2 = CNN_Attention(output_shape)(cnn2)
        att2 = keras.layers.Dropout(dropout)(att2)

        cnn3 = keras.layers.Conv1D(300, 7, padding='valid', activation='relu')(x)
        att3 = CNN_Attention(output_shape)(cnn3)
        att3 = keras.layers.Dropout(dropout)(att3)

        cnn4 = keras.layers.Conv1D(300, 9, padding='valid', activation='relu')(x)
        att4 = CNN_Attention(output_shape)(cnn4)
        att4 = keras.layers.Dropout(dropout)(att4)

        concat = keras.layers.Concatenate(axis=-1)([att1, att2, att3, att4])

        label_li = []

        for i in range(output_shape):
            label_li.append(keras.layers.Dense(1, activation='sigmoid')(concat[::, i]))
