...
icd_codes = np.asarray([toarr(clean_icd_labels(x)) for x in data['icd_codes']])
...
fb_dataset = np.stack((fb_age_test, fb_weight_test, dis_embed_mean), axis=1)
for i in range(icd_codes.shape[1]):
    fb_dataset = np.hstack((fb_dataset, icd_codes[:, [i]]))
...
top5_acc.reset_state()
top5_acc.update_state(fb_labels_test, model.predict(fb_data_dict_test))
print(f"After Feedback dataset Acc ALL: {top5_acc.result().numpy()}")
'''
for i in range(k):
