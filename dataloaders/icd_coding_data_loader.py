...
class ICD_Coding_DataLoader:
    def __init__(self, data_dir, nb_codes):
        """
        ...
        all_codes = load_vocab(f"{data_dir}TOP_{nb_codes}_CODES.csv")
        self.text_train, codes_train = load_data(f"{data_dir}train_{nb_codes}.csv")
        self.text_dev, codes_dev = load_data(f"{data_dir}dev_{nb_codes}.csv")
        self.text_test, codes_test = load_data(f"{data_dir}test_{nb_codes}.csv")
